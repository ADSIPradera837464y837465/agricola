  ...
  <table class="table table-bordered table-striped resultado ">
    ...
    <tbody>                        
      <?php foreach ($objturno as $turno): ?>      
        <tr>
          <td><input type="checkbox" name="<?php echo $turno->id  ?>"></td>
          <td><?php echo $turno->id ?></td>
          <td><?php echo $turno->descripcion ?></td>
           <td><?php echo $turno->hora_fin ?></td>
            <td><?php echo $turno->estado ?></td>
          <td><?php echo $turno->created_at ?></td>
          <?php if (strlen($turno->updated_at) > 0) : ?>
            <td><?php echo $turno->updated_at ?></td>
          <?php else : ?>        
            <td>sin actualizar</td>
          <?php endif; ?>
          <td><a class="btn btn-primary" href="<?php echo $fsConfig->getUrl() ?>index.php/metodoRiego/actualizar?id=<?php echo $turno->id ?>">
